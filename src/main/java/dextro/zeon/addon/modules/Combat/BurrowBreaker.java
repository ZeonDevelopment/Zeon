...
public class BurrowBreaker extends Module {
    ...
    public BurrowBreaker() {
        super(Zeon.Combat, "burrow-breaker", "Break player's surround.");
    }
    ...
    public void onActivate() {
        ...
        if(c.get() == BMode.Normal) {
            ...
            BlockPos mineTarget = target.getBlockPos();

            if (target == null || mineTarget == null) {
                if (chatInfo.get()) error("Target block not found... disabling.");
            } else {
                if (chatInfo.get()) info("Breaking burrow " + target.getGameProfile().getName());
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if(c.get() == BMode.Always) {
            ...
            if (b.get() == Mode.Normal) {
                BlockPos mneTarget = target.getBlockPos();
                mine(mneTarget);
            }
            else if (b.get() == Mode.Fast && (BlockUtilsWorld.getBlock(target.getBlockPos()) != Blocks.BEDROCK) && (BlockUtilsWorld.getBlock(target.getBlockPos()) != Blocks.COBWEB)) {
